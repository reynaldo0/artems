...
const Paintings = () => {
    ...
    const filteredCards = CardLeo.filter(card => card.year === selectedYear).slice(0, 4);
        ...
        <div className="relative max-w-screen-2xl mx-auto py-12 md:py-24 w-full flex flex-col items-center px-5">
            <div className="text-center mb-12">
                ...
                <div className="bg-white/80 text-primary bg-opacity-75 rounded-3xl shadow w-full md:max-w-5xl text-center p-1 md:p-5 my-8 md:my-10">
                    <div className="flex flex-wrap gap-2 py-1 px-3 justify-center md:gap-4 text-sm md:text-lg lg:text-xl">
                        {['1470s', '1480s', '1490s', '1500s'].map(year => (
                            <button
                                key={year}
                                onClick={() => setSelectedYear(year)}
                                className={`py-2 px-4 border-none cursor-pointer transition-colors duration-300 ease-in-out ${selectedYear === year ? 'bg-primary text-white rounded-lg' : 'bg-transparent rounded-lg hover:bg-gray-300'}`}
                            >
                                {year}
                            </button>
                        ))}
